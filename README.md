...
Node.js 20.9.0以上が必要です。リポジトリのルートで依存関係を入れ、環境ファイルを作成します。
...
`.env.local`を開き、自分の`GEMINI_API_KEY`と`TYPESAFE_API_KEY`を設定してください。キー値はGitに追加・コミットしないでください。キーを設定しなくても手動での記録・編集・削除は利用できますが、AI記録と音声文字起こしは使えません。
...
Web MVP実装済み。Gemini/Jev/文字起こしの実API確認は未完了。
