...
症状の種類ごとに独立して集計する。正規化ラベルがある症状はそのカテゴリを使い、ない場合は記録のlabelを使う。同じ症状グループの全症状記録が分母となり、要因記録がない症状も分母に含める。各症状記録について、症状時刻の6時間前ちょうどから症状時刻より前までのmeal/contextを対象とする。同じ食事または生活カテゴリがその区間に複数回あっても、その症状記録では1回として数える。mealはlabel、contextは正規化ラベルがあればそのカテゴリ、なければlabelを集計キーにする。`other`とsymptomは要因に含めない。全履歴を対象とし、Insight専用のDB tableは作らない。
...
利用者がご自身のPC上でアプリを動かす想定で、ブラウザーから利用できるマイクで録音する。PC/mobileのユーザーエージェント判定で録音を禁止する要件ではない。通常の録音UIはブラウザー側で最大60秒に制限し、Node runtime routeは受信音声を最大10 MiBに制限する。音声は文字起こしのためGeminiへ一時アップロードし、処理後にFiles APIから削除を試みる。削除に失敗した場合は利用者にその旨を知らせる。音声をSQLiteまたはローカルディスクへ保存しない。文字起こし結果は既存の自然文入力欄へ追記し、ユーザーが編集して「記録する」を押すまでevent抽出を行わない。文字起こしの失敗時も既存の入力下書きを保持する。ファイルアップロードUIは提供しない。起動手順は[README](../README.md)、技術詳細は[開発手順](development.md)を参照する。
...
1. 自然文入力
2. 利用者が自分のPC上で動かすブラウザーから、マイク録音の文字起こしを既存入力欄へ追記
3. LLMがevent候補を抽出
4. Jevが適切な箇所を分類 / 検証
5. Zod validation
6. 必要箇所を確認・修正
